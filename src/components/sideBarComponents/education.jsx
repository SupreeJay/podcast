...
const EducationDisplay = () => {
    ...
    const handleFavoriteClick = (podcast) => {
        ...
        // Show popup only if adding to favorites
        if (!isCurrentlyFav) {
    ...
    useEffect(() => {
        const getPodcasts = async () => {
            const itunesUrl = 'https://itunes.apple.com/search?term=education&entity=podcastEpisode&limit=100genreId=1304';
            try {
                const response = await fetch(itunesUrl);
                const data = await response.json();
                if (data.results) {
                    // No need to check localStorage here; Context handles it globally
                    const formattedData = data.results.map((item) => {
                        ...
                        return {
                            ...
                            tag: item.primaryGenreName || "Education",
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", color: "white" }}>
                ...
                <tbody>
                    {podcasts.map((podcast, index) => {
                        // 4. Determine heart color by checking if ID exists in global favorites array
